 ...
 * Module
 * File             BulkActionSetBatchesSchedule.java
...
/**
 * Description  Fetches and schedules batches
 * Author       Ranga
 * Type         BulkActionSetBatchesSchedule
 */
public class BulkActionSetBatchesSchedule extends BulkActionResultSet implements BulkActionSetBatchesInterface
{
    public BulkActionSetBatchesSchedule(String actionType, IDfSession dfSession)
    {
	...
	public boolean processPrecondition()
	{
		...
		HelperClass.porticoOutput(0, "BulkActionSetBatchesSchedule-processPrecondition()");
		...
		try
		{
			...
			if(m_hashData != null && m_hashData.size() > 0)
			{
                ...
                while(iterate.hasNext())
                {
            		...
            		HelperClass.porticoOutput(0, "BulkActionSetBatchesSchedule-processPrecondition()-currentObjectId="+currentObjectId);
...
/*---------------------------------Start changes for action 1 of 3 -------------------------------------------------*/
            		        HelperClass.porticoOutput(0, "BulkActionSetBatchesSchedule-processPrecondition()-currentObjectId((String)currentAttrHash.get(P_ON_HOLD))="+(String)currentAttrHash.get(SubmissionBatchObjectListWithMyBatches.P_ON_HOLD));
                            ...
                            {
            		            HelperClass.porticoOutput(0, "BulkActionSetBatchesSchedule-processPrecondition()-currentObjectId(p_onhold is 0)="+currentObjectId);
                                ...
                                if(QcHelperClass.isValidScheduleAction(m_dfSession, currentObjectId, addlnInfo) == true)
                                {
									isValid = true;
...
/*---------------------------------End changes for action 1 of 3 -------------------------------------------------*/
    				    ...
    				    {
							isSuccessful = false;
                			HelperClass.porticoOutput(1, "Exception in BulkActionSetBatchesSchedule-processPrecondition()-objectName="+objectName+","+"exception="+ein.toString());
    						ein.printStackTrace();
    					}
    					finally
    					{
		...
		{
			isSuccessful = false;
   			HelperClass.porticoOutput(1, "Exception in BulkActionSetBatchesSchedule-processPrecondition()-"+"exception="+e.toString());
			e.printStackTrace();
		}
		finally
		{
			HelperClass.porticoOutput(0, "BulkActionSetBatchesSchedule-processPrecondition()-finally");
		}
	...
	public boolean processUpdate(Hashtable addlnData)
	{
		...
		HelperClass.porticoOutput(0, "BulkActionSetBatchesSchedule-processUpdate()");
...
/*---------------------------------End changes for action 2 of 3 -------------------------------------------------*/
   				    ...
   				    {
						isSuccessful = false;
               			HelperClass.porticoOutput(1, "Exception in BulkActionSetBatchesSchedule-processUpdate()-objectName="+objectName+","+"exception="+ein.toString());
   						ein.printStackTrace();
   					}
   					finally
   					{
...
/*---------------------------------End changes for action 3 of 3 -------------------------------------------------*/
		...
		{
			isSuccessful = false;
   			HelperClass.porticoOutput(1, "Exception in BulkActionSetBatchesSchedule-processUpdate()-"+"exception="+e.toString());
			e.printStackTrace();
		}
		finally
		{
			HelperClass.porticoOutput(0, "BulkActionSetBatchesSchedule-processUpdate()-finally");
		}
