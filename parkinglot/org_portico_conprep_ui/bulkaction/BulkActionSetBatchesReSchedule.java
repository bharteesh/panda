 ...
 * Module
 * File             BulkActionSetBatchesReSchedule.java
 ...
 * Author       Ranga
 * Type         BulkActionSetBatchesReSchedule
 */
public class BulkActionSetBatchesReSchedule extends BulkActionResultSet implements BulkActionSetBatchesInterface
{
    public BulkActionSetBatchesReSchedule(String actionType, IDfSession dfSession)
    {
	...
	public boolean processPrecondition()
	{
		...
		HelperClass.porticoOutput(0, "BulkActionSetBatchesReSchedule-processPrecondition()");
		...
		try
		{
			...
			if(m_hashData != null && m_hashData.size() > 0)
			{
                ...
                while(iterate.hasNext())
                {
            		...
            		HelperClass.porticoOutput(0, "BulkActionSetBatchesReSchedule-processPrecondition()-currentObjectId="+currentObjectId);
...
/*---------------------------------Start changes for action 1 of 3 -------------------------------------------------*/
            		        HelperClass.porticoOutput(0, "BulkActionSetBatchesReSchedule-processPrecondition()-currentObjectId((String)currentAttrHash.get(P_ON_HOLD))="+(String)currentAttrHash.get(SubmissionBatchObjectListWithMyBatches.P_ON_HOLD));
                            ...
                            {
            		            HelperClass.porticoOutput(0, "BulkActionSetBatchesReSchedule-processPrecondition()-currentObjectId(p_onhold is 0)="+currentObjectId);
                                ...
                                if(QcHelperClass.isValidReScheduleAction(m_dfSession, currentObjectId, addlnInfo) == true)
                                {
									isValid = true;
...
/*---------------------------------End changes for action 1 of 3 -------------------------------------------------*/
    				    ...
    				    {
							isSuccessful = false;
                			HelperClass.porticoOutput(1, "Exception in BulkActionSetBatchesReSchedule-processPrecondition()-objectName="+objectName+","+"exception="+ein.toString());
    						ein.printStackTrace();
    					}
    					finally
    					{
		...
		{
			isSuccessful = false;
   			HelperClass.porticoOutput(1, "Exception in BulkActionSetBatchesReSchedule-processPrecondition()-"+"exception="+e.toString());
			e.printStackTrace();
		}
		finally
		{
			HelperClass.porticoOutput(0, "BulkActionSetBatchesReSchedule-processPrecondition()-finally");
		}
	...
	public boolean processUpdate(Hashtable addlnData)
	{
		...
		HelperClass.porticoOutput(0, "BulkActionSetBatchesReSchedule-processUpdate()");
		...
		try
		{
			...
			if(m_successfulPreconditionBatchList != null && m_successfulPreconditionBatchList.size() > 0)
			{
                ...
                while(iterate.hasNext())
		        {
                    ...
                    try
                    {
						if(addlnData.containsKey(BulkAction.START_DATE))
						{
                            Date startDate = (Date)addlnData.get(BulkAction.START_DATE);
...
/*---------------------------------Start changes for action 2 of 3 -------------------------------------------------*/
                                ...
                                iDfSysObject.setString(HelperClassConstants.BATCH_STATE, HelperClassConstants.QUEUED); // Already the batch is in 'SCHEDULED' state, just in case let us have this
                                iDfSysObject.setTime("p_sched_timestamp", dftime);
        			            iDfSysObject.save();
/*---------------------------------End changes for action 2 of 3 -------------------------------------------------*/
   				    ...
   				    {
						isSuccessful = false;
               			HelperClass.porticoOutput(1, "Exception in BulkActionSetBatchesReSchedule-processUpdate()-objectName="+objectName+","+"exception="+ein.toString());
   						ein.printStackTrace();
   					}
   					finally
   					{
...
/*---------------------------------End changes for action 3 of 3 -------------------------------------------------*/
		...
		{
			isSuccessful = false;
   			HelperClass.porticoOutput(1, "Exception in BulkActionSetBatchesReSchedule-processUpdate()-"+"exception="+e.toString());
			e.printStackTrace();
		}
		finally
		{
			HelperClass.porticoOutput(0, "BulkActionSetBatchesReSchedule-processUpdate()-finally");
		}
