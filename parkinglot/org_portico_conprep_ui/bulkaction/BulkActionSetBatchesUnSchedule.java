 ...
 * Module
 * File             BulkActionSetBatchesUnSchedule.java
 ...
 * Author       Ranga
 * Type         BulkActionSetBatchesUnSchedule
 * Note         It is possible that this action may happen at the same time when the Conprep scheduler would be picking
 *              the affected Batch, in that case Unschedule on that Batch would be late and the Batch would have got into the workflow
 *              activity, it could also result in version mismatch on the affected Batch while taking this action. Stiil, the
 *              next Batch would be picked up to work on this action.
 */
public class BulkActionSetBatchesUnSchedule extends BulkActionResultSet implements BulkActionSetBatchesInterface
{
    public BulkActionSetBatchesUnSchedule(String actionType, IDfSession dfSession)
    {
	...
	public boolean processPrecondition()
	{
		...
		HelperClass.porticoOutput(0, "BulkActionSetBatchesUnSchedule-processPrecondition()");
		...
		try
		{
			...
			if(m_hashData != null && m_hashData.size() > 0)
			{
                ...
                while(iterate.hasNext())
                {
            		...
            		HelperClass.porticoOutput(0, "BulkActionSetBatchesUnSchedule-processPrecondition()-currentObjectId="+currentObjectId);
...
/*---------------------------------Start changes for action 1 of 3 -------------------------------------------------*/
            		        HelperClass.porticoOutput(0, "BulkActionSetBatchesUnSchedule-processPrecondition()-currentObjectId((String)currentAttrHash.get(P_ON_HOLD))="+(String)currentAttrHash.get(SubmissionBatchObjectListWithMyBatches.P_ON_HOLD));
                            ...
                            {
            		            HelperClass.porticoOutput(0, "BulkActionSetBatchesUnSchedule-processPrecondition()-currentObjectId(p_onhold is 0)="+currentObjectId);
                                ...
                                if(QcHelperClass.isValidUnScheduleAction(m_dfSession, currentObjectId, addlnInfo) == true)
                                {
									isValid = true;
...
/*---------------------------------End changes for action 1 of 3 -------------------------------------------------*/
    				    ...
    				    {
							isSuccessful = false;
                			HelperClass.porticoOutput(1, "Exception in BulkActionSetBatchesUnSchedule-processPrecondition()-objectName="+objectName+","+"exception="+ein.toString());
    						ein.printStackTrace();
    					}
    					finally
    					{
		...
		{
			isSuccessful = false;
   			HelperClass.porticoOutput(1, "Exception in BulkActionSetBatchesUnSchedule-processPrecondition()-"+"exception="+e.toString());
			e.printStackTrace();
		}
		finally
		{
			HelperClass.porticoOutput(0, "BulkActionSetBatchesUnSchedule-processPrecondition()-finally");
		}
	...
	public boolean processUpdate(Hashtable addlnData)
	{
		...
		HelperClass.porticoOutput(0, "BulkActionSetBatchesUnSchedule-processUpdate()");
...
/*---------------------------------Start changes for action 2 of 3 -------------------------------------------------*/
                        ...
                        iDfSysObject.setTime("p_sched_timestamp", null);
   			            iDfSysObject.save();
/*---------------------------------End changes for action 2 of 3 -------------------------------------------------*/
   				    ...
   				    {
						isSuccessful = false;
               			HelperClass.porticoOutput(1, "Exception in BulkActionSetBatchesUnSchedule-processUpdate()-objectName="+objectName+","+"exception="+ein.toString());
   						ein.printStackTrace();
   					}
   					finally
   					{
...
/*---------------------------------End changes for action 3 of 3 -------------------------------------------------*/
		...
		{
			isSuccessful = false;
   			HelperClass.porticoOutput(1, "Exception in BulkActionSetBatchesUnSchedule-processUpdate()-"+"exception="+e.toString());
			e.printStackTrace();
		}
		finally
		{
			HelperClass.porticoOutput(0, "BulkActionSetBatchesUnSchedule-processUpdate()-finally");
		}
