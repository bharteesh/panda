...
public class SetHold extends Component{
    ...
    public void onInit(ArgumentList args){
        ...
        HelperClass.porticoOutput("SetHold-onInit()-Argument batchId=" + m_batch_id);
        initializeData();
    ...
    {
		isInitialized = true;
        HelperClass.porticoOutput(0, "SetHold-initializeData-Start");
		try
		{
		...
		{
			isInitialized = false;
	        HelperClass.porticoOutput(1, "Exception in SetHold-initializeData="+e.getMessage());
	        e.printStackTrace();
		}
		finally
		{
		}
        HelperClass.porticoOutput(0, "SetHold-initializeData-End");
	}
	...
	public void callErrorMessageService(boolean status, String msgText)
	{
		if(status)
		{
			setReturnError("MSG_SET_HOLD_ACTION_SUCCESS", null, null);
			MessageService.addMessage(this, "MSG_SET_HOLD_ACTION_SUCCESS");
		}
		else
		{
			setReturnError("MSG_SET_HOLD_ACTION_FAILED", null, null);
			ErrorMessageService.getService().setNonFatalError(this, "MSG_SET_HOLD_ACTION_FAILED", null);
		}
	}
